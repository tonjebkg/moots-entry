...
function isoToUSDate(isoString: string): string {
  if (!isoString) return ''
...
function isoToTimeString(isoString: string): string {
  ...
  const hours12 = h % 12 || 12
  const pad = (n: number) => (n < 10 ? `0${n}` : `${n}`)
  return `${hours12}:${pad(m)} ${ampm}`
}

export function EditEventModal({ eventId, onClose, onSuccess }: EditEventModalProps) {
  ...
  useEffect(() => {
    async function fetchEvent() {
      try {
        ...
        setEvName(data.title || '')
        setEvTimezone(data.timezone || 'America/New_York')
        setEvStartDate(isoToUSDate(data.start_date))
        setEvStartTime(isoToTimeString(data.start_date))
        setEvEndDate(data.end_date ? isoToUSDate(data.end_date) : '')
        setEvEndTime(data.end_date ? isoToTimeString(data.end_date) : '3:00 PM')
        setEvEventUrl(data.event_url || '')
        setEvIsPrivate(data.is_private ?? false)
        setEvApproveMode(data.approve_mode || 'MANUAL')
        setEvStatus(data.status || 'DRAFT')

        if (data.location && typeof data.location === 'object') {
          setEvLocationVenue(data.location.venue_name || '')
          setEvLocationStreet(data.location.street_address || '')
          setEvLocationCity(data.location.city || '')
          setEvLocationState(data.location.state_province || '')
        }
      } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-6 overflow-y-auto">
      <div className="bg-white rounded-card shadow-panel w-full max-w-3xl p-6 space-y-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-semibold font-display text-brand-charcoal">Edit Event</h2>
          <button
            onClick={onClose}
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <Loader2 size={24} className="animate-spin text-brand-terracotta" />
            <span className="ml-2 text-sm text-ui-tertiary">Loading event...</span>
          </div>
        ) : (
          <>
            {/* Form */}

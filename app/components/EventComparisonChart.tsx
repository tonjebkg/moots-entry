...
export function EventComparisonChart({ events }: EventComparisonChartProps) {
  ...
  const colors = ['#0f3460', '#059669'];

  return (
    <div className="bg-white border border-[#e1e4e8] rounded-lg p-6 shadow-sm">
      <h3 className="font-semibold text-[#1a1a2e] mb-4">Event Comparison</h3>
      ...
      <div className="flex gap-4 mb-4">
        {events.map((evt, i) => (
          <div key={evt.event_id} className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-[#4a4a5e]">{evt.event_title}</span>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-4">
        {metricKeys.map(({ key, label, unit }) => {
          ...
          return (
            <div key={key}>
              <div className="text-xs font-semibold text-[#6e6e7e] mb-1">{label}</div>
              <div className="space-y-1">
                {events.map((evt, i) => {
                  ...
                  return (
                    <div key={evt.event_id} className="flex items-center gap-2">
                      <div className="flex-1 h-6 bg-[#f0f2f5] rounded overflow-hidden">
                        <div

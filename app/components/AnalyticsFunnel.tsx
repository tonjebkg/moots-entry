...
export function AnalyticsFunnel({ stages }: AnalyticsFunnelProps) {
  ...
  return (
    <div className="bg-white border border-ui-border rounded-lg p-6 shadow-sm">
      <h3 className="font-semibold text-brand-charcoal mb-4">Event Funnel</h3>
          ...
          return (
            <div key={stage.key} className="flex items-center gap-3">
              <div className="w-24 shrink-0 text-right">
                <span className="text-xs font-semibold text-ui-tertiary">{stage.label}</span>
              </div>
              <div className="flex-1 relative">
                <div className="h-8 bg-brand-cream rounded-md overflow-hidden">
                  <div
              ...
              <div className="w-12 shrink-0 text-right">
                {conversionRate !== null && (
                  <span className="text-xs text-ui-tertiary">{conversionRate}%</span>
                )}
              </div>
            </div>
          );
